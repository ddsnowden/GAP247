<?php 
	...
	require_once $root.'/assets/php/class/HeadOfficeTempCall.class.php';
	$headTempData = new HeadOfficeTempCalls();
    //Recall for existing calls
	$recall = $headTempData->recall($DBH);
	$currentPage = "head office temp call";
?>
...
$(document).ready(function(){
    $("#branch").val("head_office");
    $("#branch").prop("readonly", true).prop("selected", true);
});
</script>
...
<script type="text/javascript">var callType = 'head office temp call';</script>
<!-- Start main content of the page -->
<div class="row form">
	<div class="col-sm-8 col-md-8 col-lg-5">
        
		<form method="POST" action="/assets/php/HeadOfficeTempCall.php" role="form" class="form-horizontal" >
		    <div class="row">
    			...
    			<input class="hidden" type='hidden' name='type' value="head office temp call" />
                ...
                <div class="col-sm-6 labright">
                    <label for="branch">Trying to contact:</label>
                </div>
                <div class="col-sm-10">
                    <select class="form-control input" name='contact'>
                        <option value='' <?=(isset($_SESSION['form']['contact']) && $_SESSION['form']['contact'] == '' ? 'selected' : '')?> ></option>
                        <?php
                            foreach ($headTempData->contacts($DBH) as $key) { 
                                $name = $key['firstName']." ".$key['lastName'];
                        ?>
                                <option value="<?php echo $name; ?>" <?=(isset($_SESSION['form']['contact']) && $_SESSION['form']['contact'] == $name ? 'selected' : '')?> ><?php echo $name." - ".$key['position']; ?></option>
                        <?php
                            }
                        ?>
                        </select>
                ...
                <div id="tempDetails">
                    <input class="form-control input" name="tempID" id="tempID" type="hidden" value="<?php if (isset($_SESSION['form']['tempID'])) echo htmlspecialchars($_SESSION['form']['tempID'], ENT_QUOTES); ?>" required />
